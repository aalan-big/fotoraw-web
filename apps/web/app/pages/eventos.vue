...
<template>
  <div class="mx-auto max-w-6xl px-4 py-10">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      ...
      <form class="flex gap-2" @submit.prevent="buscar">
        <input
          ...
          class="h-10 w-full rounded-lg border border-borda-2 bg-card px-3 text-sm outline-none placeholder:text-texto-3 focus:border-primaria md:w-72"
        />
    ...
    <div v-else class="card mt-8 px-6 py-14 text-center">
      ...
      <p class="mt-1 text-sm text-texto-3">Tente outro nome ou peça o link direto pro fotógrafo.</p>
    </div>
  </div>
</template>
